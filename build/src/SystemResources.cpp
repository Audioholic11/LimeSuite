...
std::string lime::getLimeSuiteRoot(void)
{
    ...
    return "/home/erik/prefix/default";
}
...
std::string lime::locateImageResource(const std::string &name)
{
    ...
    {
        const std::string fullPath(searchPath + "/18.06/" + name);
...
{
    const std::string destDir(lime::getAppDataDirectory() + "/images/18.06");
    const std::string destFile(destDir + "/" + name);
    const std::string sourceUrl("http://downloads.myriadrf.org/project/limesuite/18.06/" + name);
